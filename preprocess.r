...
df <- df |>
	...
	mutate(
		response = response |>
			trimws(),
		word = gsub('^“?(.*?)”? \\[(.*?)\\]$', '\\1', word_IPA),
		word_IPA = gsub('^“?(.*?)”? \\[(.*?)\\]$', '\\2', word_IPA),
		response_accuracy = as.numeric(tolower(response) == tolower(word))
	) |>
	left_join(demographics) |>
	select(
		participant, age, college, item, word,
		response, response_time, response_accuracy, everything()
	)
